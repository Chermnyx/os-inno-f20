...
int main(int argc, char const* argv[]) {
  ...
  // after some time the code runs into deadlock because of race condition
  // it occurs because there are no synchronization primitives
  // eventually the consumer and calls sleep() (because c = 0)
  // menawhile the producer fills the buffer and sleeps as well, but the
  // consumer haven't called sleep() yet after that both processes wait for each
  // other (deadlock)
  return 0;
}
